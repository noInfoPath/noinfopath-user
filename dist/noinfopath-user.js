...
(function (angular, undefined) {
	...
	.directive("noUserMenu", [function () {
		return {
			...
			controller: ["$scope", "$uibModal", "noConfig", "noLoginService", "noTemplateCache", "noPrompt", function ($scope, $uibModal, noConfig, noLoginService, noTemplateCache, noPrompt) {
				...
				noConfig.whenReady("config.json")
					.then(function () {
						var localStoresExists = noConfig.current.localStores ? true : false,
							...
							logoutTemplate = '<div class="modal-body text-center">Are you sure you would like to logout?</div></div>',
							url = noConfig.current.noUser ? noConfig.current.noUser.logoutTemplate : null;
									...
									function (e) {
										switch($(e.target)
											...
											case "clearDb":
												...
												break;
											case "keepDb":
												noLoginService.logout(noConfig.current.localStores, false);
												location.href = "/";
												break;
											case "cancel":
												break;
											default:
												console.warn("noUserMenu: Invalid value provided. Supported values: clearDb, keepDb, cancel");
												break;
										}
									}, {
										height: "15%",
										messageIsTemplateUrl: true,
										showFooter: false
									}
								);
							} else {
